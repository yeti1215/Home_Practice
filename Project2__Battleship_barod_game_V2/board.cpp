...
void PlayerBoard::setBoard2()
{
 ...
 void PlayerBoard::gameguess1(){
    guessR=rand()%+10; //random row range 0-9
    guessC=rand()%+10;//random column range 0-9
          if(boardAry1[guessR][guessC]==1) //if guess spot is '1' means hit
    {
         
    boardAry1[guessR][guessC]=0;} // then need set this spot to '0'
}
 
  void PlayerBoard::gameguess2(){
    guessR=rand()%+10; //random row range 0-9
    guessC=rand()%+10;//random column range 0-9
          if(boardAry2[guessR][guessC]==1) //if guess spot is '1' means hit
    {
         
    boardAry2[guessR][guessC]=0;} // then need set this spot to '0'
}
 
 
 bool PlayerBoard::resultCheck( int boardAry[10][10]){
    
    bool result=true; 
    for(int i=0; i<row; i++){
        for (int j=0; j<col; j++){
            if(boardAry[i][j]==1) result=false;// return true if still has '1' in game board,
