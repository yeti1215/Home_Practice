 ...
 * Project2: Battle ship board game Version 7( exceptions templates /copy constructor not working)
 */
...
////This is a function template to compare two parameters which is larger
////in this game we will compare who wins more
//template <class T1, class T2>
//int largest(const T1 val1, const T1 val2)
//{
//    if(val1>val2){
//        return val1;
//    }
...
class Person{
    public:
        ...
        void showPerson(){
            cout<<"Player name: "<<this->Name<< " Score = "<<this->Score<<endl;
        }
};
//template<class T1>
//void showperson(T1 person)
//{
//    cout<<person<<endl;
//}
...
int main(int argc, char** argv) {
   ...
   char *name; //ask for the players name
   ...
   // Player player[arySize];
    ...
    cin.getline(name,20); //the name size is 20
    pname1->setName(name);
    ...
    //release the memory in class,use template print
    cout<<"use template class Print out first players data "<<endl;
        Person<string,int>person1(pname1->p_name,pscore1->p_socre);//???????????????????????
        person1.showPerson();
    
   
    cout<<"use template class Print out second players data "<<endl;
        Person<string,int>person2(pname2->p_name,pscore2->p_socre);//???????????????????????
        person2.showPerson();
