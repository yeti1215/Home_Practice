 ...
 * Purpose: project 2# Battle ship Version 2
 */
...
int main(int argc, char** argv) {
  ...
  do{int guessR, guessC;  //guessR= guess row number; guessC= guess Column number;
     ...
     else if(!guess(myBoard,guessR,guessC)){
         cout<<"MISS"; 
         myBoard[guessR][guessC]="0";}
     
      displayBoard(myBoard, ROWS);
      // searching for sink ship
       searchSink1( myBoard);
        searchSink2( myBoard);
         searchSink3( myBoard);
          searchSink4( myBoard);
         
   }while ( !searchSink1( myBoard) || !searchSink2( myBoard) || !searchSink3( myBoard) || !searchSink4( myBoard));
